...
.
.TH "QUEUE_MESSAGES_DELETE-MESSAGES" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
queue_messages_delete-messages \- 
.
...
.SH DESCRIPTION
.sp
Deletes multiple messages from the queue or the consumer group. Only messages from the same queue/consumer group can be deleted at once. You must use the \fI\%messages endpoint\fP <\fBhttps://docs.cloud.oracle.com/iaas/Content/queue/messages.htm#messages__messages-endpoint\fP> to delete messages. The messages endpoint may be different for different queues. Use 
.nf
\(ga\(ga
.fi
...
.nf
.ft C
oci queue messages delete\-messages [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    oci queue messages delete\-messages \-\-entries file://entries.json \-\-queue\-id $queue_id
.ft P
.fi
.UNINDENT
.UNINDENT
