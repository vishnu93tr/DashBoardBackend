...
.
.TH "IAM_MFA-TOTP-DEVICE" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
iam_mfa-totp-device \- 
.
...
.SH DESCRIPTION
.sp
Users can enable multi\-factor authentication (MFA) for their own user accounts. After MFA is enabled, the user is prompted for a time\-based one\-time password (TOTP) to authenticate before they can sign in to the Console. To enable multi\-factor authentication, the user must register a mobile device with a TOTP authenticator app installed. The registration process creates the \fIMfaTotpDevice\fP object. The registration process requires interaction with the Console and cannot be completed programmatically. For more information, see \fI\%Managing Multi\-Factor Authentication\fP <\fBhttps://docs.cloud.oracle.com/Content/Identity/mfa/understand-multi-factor-authentication.htm\fP>\&.
...
.nf
.ft C
oci iam mfa\-totp\-device activate [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    mfa_totp_device_id=$(oci iam mfa\-totp\-device create \-\-user\-id $user_id \-\-query data.id \-\-raw\-output)

    oci iam mfa\-totp\-device activate \-\-mfa\-totp\-device\-id $mfa_totp_device_id \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
oci iam mfa\-totp\-device create [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    oci iam mfa\-totp\-device create \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
oci iam mfa\-totp\-device delete [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    oci iam mfa\-totp\-device delete \-\-mfa\-totp\-device\-id $mfa_totp_device_id \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
.SS \fBgenerate\-totp\-seed\fP
.INDENT 0.0
.IP \(bu 2
\fI\%Description\fP
.IP \(bu 2
...
.nf
.ft C
oci iam mfa\-totp\-device generate\-totp\-seed [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    oci iam mfa\-totp\-device generate\-totp\-seed \-\-mfa\-totp\-device\-id $mfa_totp_device_id \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
oci iam mfa\-totp\-device get [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    oci iam mfa\-totp\-device get \-\-mfa\-totp\-device\-id $mfa_totp_device_id \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    oci iam mfa\-totp\-device list \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
