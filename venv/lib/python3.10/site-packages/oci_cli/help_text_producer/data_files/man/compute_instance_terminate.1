...
.
.TH "COMPUTE_INSTANCE_TERMINATE" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
...
.sp
To preserve the boot volume associated with the instance, specify \fItrue\fP for \fIPreserveBootVolumeQueryParam\fP\&. To delete the boot volume when the instance is deleted, specify \fIfalse\fP or do not specify a value for \fIPreserveBootVolumeQueryParam\fP\&.
.sp
To preserve data volumes created with the instance, specify \fItrue\fP or do not specify a value for \fIPreserveDataVolumesQueryParam\fP\&. To delete the data volumes when the instance itself is deleted, specify \fIfalse\fP for \fIPreserveDataVolumesQueryParam\fP\&.
.sp
This is an asynchronous operation. The instance’s \fIlifecycleState\fP changes to TERMINATING temporarily until the instance is completely deleted. After the instance is deleted, the record remains visible in the list of instances with the state TERMINATED for at least 12 hours, but no further action is needed.
.SH USAGE
.INDENT 0.0
.INDENT 3.5
.sp
...
.INDENT 0.0
.TP
.B \-\-preserve\-boot\-volume [boolean]
.UNINDENT
.sp
Specifies whether to delete or preserve the boot volume when terminating an instance. When set to \fItrue\fP, the boot volume is preserved. The default value is \fIfalse\fP\&.
.INDENT 0.0
.TP
.B \-\-preserve\-data\-volumes\-created\-at\-launch [boolean]
.UNINDENT
.sp
Specifies whether to delete or preserve the data volumes created during launch when terminating an instance. When set to \fItrue\fP, the data volumes are preserved. The default value is \fItrue\fP\&.
.INDENT 0.0
.TP
