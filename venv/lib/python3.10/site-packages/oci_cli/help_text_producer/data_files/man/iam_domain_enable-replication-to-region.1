...
.
.TH "IAM_DOMAIN_ENABLE-REPLICATION-TO-REGION" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
iam_domain_enable-replication-to-region \- 
.
...
.SH DESCRIPTION
.sp
(For tenancies that support identity domains) Replicates the identity domain to a new region (provided that the region is the tenancy home region or other region that the tenancy subscribes to). You can only replicate identity domains that are in an ACTIVE \fIlifecycleState\fP and not currently updating or already replicating. You also can only trigger the replication of secondary identity domains. The default identity domain is automatically replicated to all regions that the tenancy subscribes to.
.sp
After you send the request, the \fIstate\fP of the identity domain in the replica region is set to ENABLING_REPLICATION. When the operation completes, the \fIstate\fP is set to REPLICATION_ENABLED.
.sp
...
.nf
.ft C
oci iam domain enable\-replication\-to\-region [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.UNINDENT
.sp
A region to which you want identity domain replication to occur. See \fI\%Regions and Availability Domains\fP <\fBhttps://docs.cloud.oracle.com/Content/General/Concepts/regions.htm\fP> for the full list of supported region names.
.sp
Example:
.INDENT 0.0
.INDENT 3.5
...
.ft C
    ...
    oci iam domain enable\-replication\-to\-region \-\-domain\-id $domain_id
.ft P
.fi
.UNINDENT
.UNINDENT
