...
.
.TH "IAM_SMTP-CREDENTIAL" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
iam_smtp-credential \- 
.
...
.sp
\fBNote:\fP The credential set is always an Oracle\-generated SMTP user name and password pair; you cannot designate the SMTP user name or the SMTP password.
.sp
...
.SS Description
.sp
Creates a new SMTP credential for the specified user. An SMTP credential has an SMTP user name and an SMTP password. You must specify a \fIdescription\fP for the SMTP credential (although it can be an empty string). It does not have to be unique, and you can change it anytime with \fI\%UpdateSmtpCredential\fP <\fBhttps://docs.cloud.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/iam/smtp-credential/update.html\fP>\&.
.SS Usage
.INDENT 0.0
.INDENT 3.5
.sp
...
.ft C
    ...
    oci iam smtp\-credential create \-\-description $description \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
oci iam smtp\-credential delete [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    smtp_credential_id=$(oci iam smtp\-credential create \-\-description $description \-\-user\-id $user_id \-\-query data.id \-\-raw\-output)

    oci iam smtp\-credential delete \-\-smtp\-credential\-id $smtp_credential_id \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
oci iam smtp\-credential list [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    oci iam smtp\-credential update \-\-smtp\-credential\-id $smtp_credential_id \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
