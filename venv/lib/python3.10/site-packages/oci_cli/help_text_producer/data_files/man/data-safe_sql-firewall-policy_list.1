...
.
.TH "DATA-SAFE_SQL-FIREWALL-POLICY_LIST" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
data-safe_sql-firewall-policy_list \- 
.
...
.sp
The ListSqlFirewallPolicies operation returns only the SQL Firewall policies in the specified \fIcompartmentId\fP\&.
.sp
...
.sp
The parameter \fIcompartmentIdInSubtree\fP applies when you perform ListSqlFirewallPolicies on the \fIcompartmentId\fP passed and when it is set to true, the entire hierarchy of compartments can be returned. To get a full list of all compartments and subcompartments in the tenancy (root compartment), set the parameter \fIcompartmentIdInSubtree\fP to true and \fIaccessLevel\fP to ACCESSIBLE.
.SH USAGE
.INDENT 0.0
.INDENT 3.5
.sp
.nf
.ft C
oci data\-safe sql\-firewall\-policy list [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
    export compartment_id=<substitute\-value\-of\-compartment_id> # https://docs.cloud.oracle.com/en\-us/iaas/tools/oci\-cli/latest/oci_cli_docs/cmdref/data\-safe/sql\-firewall\-policy/list.html#cmdoption\-compartment\-id

    oci data\-safe sql\-firewall\-policy list \-\-compartment\-id $compartment_id
.ft P
.fi
.UNINDENT
.UNINDENT
