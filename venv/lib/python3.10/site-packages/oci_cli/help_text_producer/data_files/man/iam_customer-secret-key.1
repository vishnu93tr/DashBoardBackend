...
.
.TH "IAM_CUSTOMER-SECRET-KEY" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
iam_customer-secret-key \- 
.
...
.SH DESCRIPTION
.sp
A \fICustomerSecretKey\fP is an Oracle\-provided key for using the Object Storage Service’s \fI\%Amazon S3 compatible API\fP <\fBhttps://docs.cloud.oracle.com/Content/Object/Tasks/s3compatibleapi.htm\fP>\&. The key consists of a secret key/access key pair. A user can have up to two secret keys at a time.
.sp
\fBNote:\fP The secret key is always an Oracle\-generated string; you can’t change it to a string of your choice.
.sp
...
.SS Description
.sp
Creates a new secret key for the specified user. Secret keys are used for authentication with the Object Storage Service’s Amazon S3 compatible API. The secret key consists of an Access Key/Secret Key pair. For information, see \fI\%Managing User Credentials\fP <\fBhttps://docs.cloud.oracle.com/Content/Identity/access/managing-user-credentials.htm\fP>\&.
.sp
You must specify a \fIdescription\fP for the secret key (although it can be an empty string). It does not have to be unique, and you can change it anytime with \fI\%UpdateCustomerSecretKey\fP <\fBhttps://docs.cloud.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/iam/customer-secret-key/update.html\fP>\&.
.sp
Every user has permission to create a secret key for \fItheir own user ID\fP\&. An administrator in your organization does not need to write a policy to give users this ability. To compare, administrators who have permission to the tenancy can use this operation to create a secret key for any user, including themselves.
.SS Usage
.INDENT 0.0
.INDENT 3.5
.sp
...
.ft C
    ...
    oci iam customer\-secret\-key create \-\-display\-name $display_name \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
oci iam customer\-secret\-key delete [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    oci iam customer\-secret\-key delete \-\-customer\-secret\-key\-id $customer_secret_key_id \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
oci iam customer\-secret\-key list [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    oci iam customer\-secret\-key list \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
