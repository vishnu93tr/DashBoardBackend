...
.
.TH "DATA-SAFE_SQL-COLLECTION_LIST" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
data-safe_sql-collection_list \- 
.
...
.sp
The ListSqlCollections operation returns only the SQL collections in the specified \fIcompartmentId\fP\&.
.sp
...
.nf
.ft C
oci data\-safe sql\-collection list [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
COLLECTING, COMPLETED, CREATING, DELETED, DELETING, FAILED, INACTIVE, NEEDS_ATTENTION, UPDATING
.ft P
.fi
.UNINDENT
.UNINDENT
...
.UNINDENT
.sp
The field used for sorting. Only one sorting parameter order (sortOrder) can be specified. The DISPLAYNAME sort order is case sensitive.
.sp
Accepted values are:
.INDENT 0.0
.INDENT 3.5
...
.nf
.ft C
DISPLAYNAME, TIMECREATED, TIMELASTSTARTED
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
    export compartment_id=<substitute\-value\-of\-compartment_id> # https://docs.cloud.oracle.com/en\-us/iaas/tools/oci\-cli/latest/oci_cli_docs/cmdref/data\-safe/sql\-collection/list.html#cmdoption\-compartment\-id

    oci data\-safe sql\-collection list \-\-compartment\-id $compartment_id
.ft P
.fi
.UNINDENT
.UNINDENT
