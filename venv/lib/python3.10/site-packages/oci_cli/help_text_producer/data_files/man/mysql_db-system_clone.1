...
.
.TH "MYSQL_DB-SYSTEM_CLONE" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
mysql_db-system_clone \- 
.
...
.UNINDENT
.sp
The database mode indicating the types of statements that will be allowed to run in the DB system. This mode will apply only to statements run by user connections. Replicated write statements will continue to be allowed regardless of the DatabaseMode.   \- READ_WRITE (default): allow running read and write statements on the DB system;   \- READ_ONLY: only allow running read statements on the DB system.
.INDENT 0.0
.TP
