...
.
.TH "IAM_AUTH-TOKEN" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
iam_auth-token \- 
.
...
.SH DESCRIPTION
.sp
An \fIAuthToken\fP is an Oracle\-generated token string that you can use to authenticate with third\-party APIs that do not support Oracle Cloud Infrastructure’s signature\-based authentication. For example, use an \fIAuthToken\fP to authenticate with a Swift client with the Object Storage Service.
.sp
...
.sp
\fBNote:\fP The token is always an Oracle\-generated string; you can’t change it to a string of your choice.
.sp
...
.SS Description
.sp
Creates a new auth token for the specified user. For information about what auth tokens are for, see \fI\%Managing User Credentials\fP <\fBhttps://docs.cloud.oracle.com/Content/Identity/access/managing-user-credentials.htm\fP>\&.
.sp
You must specify a \fIdescription\fP for the auth token (although it can be an empty string). It does not have to be unique, and you can change it anytime with \fI\%UpdateAuthToken\fP <\fBhttps://docs.cloud.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/iam/auth-token/update.html\fP>\&.
.sp
...
.ft C
    ...
    oci iam auth\-token create \-\-description $description \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
oci iam auth\-token delete [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    auth_token_id=$(oci iam auth\-token create \-\-description $description \-\-user\-id $user_id \-\-query data.id \-\-raw\-output)

    oci iam auth\-token delete \-\-auth\-token\-id $auth_token_id \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
oci iam auth\-token list [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.nf
.ft C
oci iam auth\-token update [OPTIONS]
.ft P
.fi
.UNINDENT
.UNINDENT
...
.ft C
    ...
    oci iam auth\-token update \-\-auth\-token\-id $auth_token_id \-\-user\-id $user_id
.ft P
.fi
.UNINDENT
.UNINDENT
