...
.
.TH "RECOVERY_PROTECTED-DATABASE_CREATE" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
recovery_protected-database_create \- 
.
...
.INDENT 0.0
.TP
.B \-\-protection\-policy\-id [text]
.UNINDENT
.sp
The OCID of the protection policy associated with the protected database.
.INDENT 0.0
.TP
.B \-\-recovery\-service\-subnets [complex type]
.UNINDENT
.sp
...
.INDENT 0.0
.TP
.B \-\-database\-size\-in\-gbs [integer]
.UNINDENT
.sp
...
.UNINDENT
.sp
Defined tags for this resource. Each key is predefined and scoped to a namespace. Example: \fI{“foo\-namespace”: {“bar\-key”: “value”}}\fP\&. For more information, see \fI\%Resource Tags\fP <\fBhttps://docs.oracle.com/en-us/iaas/Content/General/Concepts/resourcetags.htm\fP>
This is a complex type whose value must be valid JSON. The value can be provided as a string on the command line or passed in as a file using
the \fI\%file://path/to/file\fP syntax.
.sp
The \fB\-\-generate\-param\-json\-input\fP option can be used to generate an example of the JSON which must be provided. We recommend storing this example
...
.INDENT 0.0
.TP
.B \-\-is\-redo\-logs\-shipped [boolean]
.UNINDENT
.sp
...
.sp
If you set the \fIopcDryRun\fP option as \fItrue\fP, then Recovery Service only performs a test run to check for any missing prerequisites or configurations required to create a protected database, and then returns error messages to warn you about any missing requirements.
.sp
If an error occurs, you can review, correct, and repeat the dry run until the \fIcreateProtectedDatabase\fP request does not return any errors.
.sp
These are the common issues that you can identify by performing a dry run of the \fIcreateProtectedDatabase\fP request:
.INDENT 0.0
.IP \(bu 2
The Recovery Service subnet has insufficient free IP addresses to support the required number of private endpoints. See, \fI\%troubleshooting\fP <\fBhttps://docs.oracle.com/en/cloud/paas/recovery-service/dbrsu/troubleshoot-backup-failures-recovery-service.html#GUID-05FA08B8-421D-4E52-B84B-7AFB84ADECF9\fP> information * Recovery Service does not have permissions to manage the network resources in a chosen compartment * Recovery Service is out of capacity. See, \fI\%Service Limits\fP <\fBhttps://docs.oracle.com/en-us/iaas/Content/General/Concepts/servicelimits.htm\fP> for more information * Recovery Service resources exceed quota limits * A protected database, having the same database ID, already exists * The specified protection policy does not exist, or it is not in an Active state * The specified Recovery Service subnet does not exist, or it is not in an Active state
.UNINDENT
.sp
See, \fI\%Prerequisites for Using Recovery Service\fP <\fBhttps://docs.oracle.com/en/cloud/paas/recovery-service/dbrsu/backup-recover-recovery-service.html#GUID-B2ABF281-DFF8-4A4E-AC85-629801AAF36A\fP> for more information.
.INDENT 0.0
.TP
