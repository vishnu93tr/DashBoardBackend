...
.
.TH "IAM_DOMAIN_DELETE" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
iam_domain_delete \- 
.
...
.SH DESCRIPTION
.sp
(For tenancies that support identity domains) Deletes an identity domain. The identity domain must have no active apps present in the underlying IDCS stripe. You must also deactivate the identity domain, rendering the \fIlifecycleState\fP of the identity domain INACTIVE. Furthermore, as the authenticated user performing the operation, you cannot be a member of the identity domain you are deleting. Lastly, you cannot delete the default identity domain. A tenancy must always have at least the default identity domain.
.sp
...
.ft C
    ...
    oci iam domain delete \-\-domain\-id $domain_id
.ft P
.fi
.UNINDENT
.UNINDENT
