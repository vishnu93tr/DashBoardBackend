...
.
.TH "NLB_BACKEND_CREATE" "1" "Apr 23, 2025" "3.54.5" "OCI CLI Command Reference"
.SH NAME
nlb_backend_create \- 
.
...
.UNINDENT
.sp
The IP address of the backend server. Example: \fI10.0.0.3\fP
.INDENT 0.0
.TP
.B \-\-is\-backup [boolean]
.UNINDENT
.sp
Whether the network load balancer should treat this server as a backup unit. If \fItrue\fP, then the network load balancer forwards no ingress traffic to this backend server unless all other backend servers not marked as “isBackup” fail the health check policy.
.sp
Example:
.INDENT 0.0
.INDENT 3.5
...
.UNINDENT
.sp
Optional unique name identifying the backend within the backend set. If not specified, then one will be generated. Example: \fIwebServer1\fP
.INDENT 0.0
.TP
.B \-\-target\-id [text]
.UNINDENT
.sp
The IP OCID/Instance OCID associated with the backend server. Example: \fIocid1.privateip..oc1.<var>&lt;unique_ID&gt;</var>\fP
.INDENT 0.0
.TP
...
.UNINDENT
.sp
The network load balancing policy weight assigned to the server. Backend servers with a higher weight receive a larger proportion of incoming traffic. For example, a server weighted ‘3’ receives three times the number of new connections as a server weighted ‘1’. For more information about network load balancer policies, see \fI\%Network Load Balancer Policies\fP <\fBhttps://docs.cloud.oracle.com/Content/NetworkLoadBalancer/introduction.htm#Policies\fP>\&.
.sp
Example:
.INDENT 0.0
.INDENT 3.5
